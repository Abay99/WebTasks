...
["maker"=>"Lada","model"=>"Granta","year"=>2017,"price"=>7000,"image"=>"http://www.kolesa.ru/uploads/2017/06/Lada-Granta-restyle-front1-1600x0-c-default.jpg"]];
	for ($row = 0; $row < 8; $row++) {
		if($_POST["lowest"]<=$cars[$row]["price"] && $_POST["highest"]>=$cars[$row]["price"]){
			$ima = $cars[$row]["image"];
			echo '<div class="card" width="250" height="200">
			<img width="150" height="120" src="' .$ima . '"/>
			<div class="description">
			<div class="title"><p >'.$cars[$row]["maker"]." ".$cars[$row]["model"].'</p></div>
			<div class="price"><p >'."$".$cars[$row]["price"].'</p></div>
			<div class="year"><p >'.$cars[$row]["year"].'</p></div>
			</div>
			</div>';
		}
	}
